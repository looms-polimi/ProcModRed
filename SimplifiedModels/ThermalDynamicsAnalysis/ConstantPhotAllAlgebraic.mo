...
model ConstantPhotAllAlgebraic
...
equation
  ...
  for i in 1:n loop
    ...
    0 = -gammaNomCold*(pOutCold/pNomCold)^alpha*(wCold[i + 1]/wNomCold)^beta*Scold/n*(Twall[i] - (Tcold[i] + Tcold[i + 1])/2) - gammaNomHot*(pOutHot/pNomHot)^alpha*(wHot[n + 1 - i]/wNomHot)^beta*Shot/n*(Twall[i] - (Thot[n + 2 - i] + Thot[n + 1 - i])/2);
  end for;
// Turbine
