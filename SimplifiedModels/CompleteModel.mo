...
equation
  ...
  pInHot - pHot = kHot/2*wHot[1];
  pHot - pOutHot = kHot/2*wHot[n + 1];

  for i in 1:n loop
    Vcold/n/(Rcold*Tcold[i+1])*der(pCold) = wCold[i] - wCold[i + 1];
    Vhot/n/(Rhot*Thot[i+1])*der(pHot) = wHot[i] - wHot[i + 1];
    ...
    pHot/Rhot/Thot[i+1]*Vhot/n*cvHot*der(Thot[i+1]) = wHot[i+1]*cpHot*(Thot[i]-Thot[i+1]) + gammaNomHot*(pOutHot/pNomHot)^alpha*(wHot[i+1]/wNomHot)^beta*Shot/n*(Twall[n+1-i] - (Thot[i]+Thot[i+1])/2);

    Mw/n*cW*der(Twall[i]) = 
      - gammaNomCold*(pOutCold/pNomCold)^alpha*(wCold[i+1]/wNomCold)^beta*Scold/n*(Twall[i] - (Tcold[i]+Tcold[i+1])/2) 
      - gammaNomHot*(pOutHot/pNomHot)^alpha*(wHot[n+1-i]/wNomHot)^beta*Shot/n*(Twall[i] - (Thot[n+2-i]+Thot[n+1-i])/2);
